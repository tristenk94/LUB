...
namespace LEGOModelImporter
{
	...
	public static class MathUtils
	{
		...
		public static Vector3 GetColumn(Matrix4x4 m, int index)
        {
            if(index > 3)
            {
                return Vector3.zero;
        ...
        public static Matrix4x4 SetTranslation(Matrix4x4 source, Vector3 position)
        {
            var m = source;
        ...
        public static Matrix4x4 RotateAround(Matrix4x4 source, Vector3 pivot, Quaternion rotation)
        {
		...
		/// <returns></returns>
		public static Quaternion AlignRotation(Matrix4x4 destination, params Vector3[] sourceAxes)
		{
			var inverseDst = destination.inverse;

			(float, Vector3, Vector3) currentSmallestAngle = (float.NegativeInfinity, Vector3.zero, Vector3.zero);

			foreach(var axis in sourceAxes)
			{
				var localAxis = inverseDst.MultiplyVector(axis);
				var snappedLocalAxis = SnapMajorAxis(localAxis, true);
				var snappedWorld = destination.MultiplyVector(snappedLocalAxis);

				// Compare dot product instead of angle.
				var dot = Vector3.Dot(axis, snappedWorld);
				if(dot > currentSmallestAngle.Item1)
                {
					currentSmallestAngle = (dot, axis, snappedWorld);
                }
			}

			if(float.IsNegativeInfinity(currentSmallestAngle.Item1))
            {
				return Quaternion.identity;
            }
			return Quaternion.FromToRotation(currentSmallestAngle.Item2, currentSmallestAngle.Item3);
		}
		...
		*/
		...
		public static bool NearNull(Vector3 v)
        {
			var epsilon = 0.1f;
		...
		public static bool ClosestLineSegmentLineSegment(Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4, out Vector3 pa, out Vector3 pb, out float mua, out float mub)
		{
			var p21 = p2 - p1;
			var p43 = p4 - p3;
			var p13 = p1 - p3;
